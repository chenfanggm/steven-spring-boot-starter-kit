...
CREATE TABLE `topic` (
  ...
  `topic_length` tinyint(3) DEFAULT 30,
  `status` tinyint(1) NOT NULL,
  `time_created` bigint(20) NOT NULL,
  `time_updated` bigint(20) NOT NULL,
...
CREATE TABLE `mailer` (
  `user_id` int(12) unsigned NOT NULL,
  `email_type` tinyint(2) unsigned NOT NULL,
  `email_id` int(12) unsigned NOT NULL,
  PRIMARY KEY (`user_id`, `email_type`, `email_id`),
  CONSTRAINT `mailer_user_id_foreign` FOREIGN KEY (`user_id`) REFERENCES `user`
  (`user_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `mailer_email_id_foreign` FOREIGN KEY (`email_id`) REFERENCES
  `email` (`email_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `email` (
  `email_id` int(12) unsigned NOT NULL AUTO_INCREMENT,
  `email_subject` text,
  `email_content` text,
  `status` tinyint(1) NOT NULL,
  `time_created` bigint(20) NOT NULL,
